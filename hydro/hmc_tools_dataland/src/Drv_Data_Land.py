...
class Drv_Data_Land:
    ...
    def computeTerrain(self, oDataTerrain):
        ...
        try:
            ...
            #-------------------------------------------------------------------------------------
            # Define filename
            sFileNameOUT_T = Lib_Data_IO_Utils.defineFileName(join(
                                                           self.oPathInfo['DataStaticOutcome'], 
                                                           self.oVarsOUT['ASCII']['Terrain']['VarSource']), 
                                                           {'domain' : self.oParamsInfo['DomainName']})
            sFileNameOUT_X = Lib_Data_IO_Utils.defineFileName(join(
                                                           self.oPathInfo['DataStaticOutcome'], 
                                                           'domain.lon.txt'), 
                                                           {'domain' : self.oParamsInfo['DomainName']})
            sFileNameOUT_Y = Lib_Data_IO_Utils.defineFileName(join(
                                                           self.oPathInfo['DataStaticOutcome'], 
                                                           'domain.lat.txt'), 
                                                           {'domain' : self.oParamsInfo['DomainName']})
            #-------------------------------------------------------------------------------------
            ...
            #-------------------------------------------------------------------------------------
            # File driver
            oFileDrv = Drv_Data_IO(sFileNameOUT_T,'w')   
            # Get write method selection
            oApps_WriteMethod = getattr(oFileDrv.oFileWorkspace, 
            ...
            # File driver
            oFileDrv = Drv_Data_IO(sFileNameOUT_X,'w')   
            # Get write method selection
            oApps_WriteMethod = getattr(oFileDrv.oFileWorkspace, 
                                      self.oVarsOUT['ASCII']['Terrain']['VarOp']['Op_Save']['Func'])
            oFileComp = self.oVarsOUT['ASCII']['Terrain']['VarOp']['Op_Save']
            # Write file
            oApps_WriteMethod(a2dGeoX, oDataTerrain.a1oGeoInfo, 'longitude')
            # Close file
            oFileDrv.oFileWorkspace.closeFile()
            
            # File driver
            oFileDrv = Drv_Data_IO(sFileNameOUT_Y,'w')   
            # Get write method selection
            oApps_WriteMethod = getattr(oFileDrv.oFileWorkspace, 
                                      self.oVarsOUT['ASCII']['Terrain']['VarOp']['Op_Save']['Func'])
            oFileComp = self.oVarsOUT['ASCII']['Terrain']['VarOp']['Op_Save']
            # Write file
            oApps_WriteMethod(a2dGeoY, oDataTerrain.a1oGeoInfo, 'latitude')
            # Close file
            oFileDrv.oFileWorkspace.closeFile()
            #-------------------------------------------------------------------------------------
    ...
    #-------------------------------------------------------------------------------------
    # Method to define nature map domain
    def computeNatureType(self, oDataTerrain, oDataVegType, oDataNature):
        
        #-------------------------------------------------------------------------------------
        # Check section
        oLogStream.info( ' ------> Compute NatureType data ... ')
        try:
            ...
            sFileNameOUT = Lib_Data_IO_Utils.defineFileName(join(
                                                           self.oPathInfo['DataStaticOutcome'], 
                                                           self.oVarsOUT['ASCII']['NatureType']['VarSource']), 
                                                           {'domain' : self.oParamsInfo['DomainName']})
            #-------------------------------------------------------------------------------------
            
            #-------------------------------------------------------------------------------------
            # Initialize nature map
            a2iNature = np.zeros([oDataTerrain.a2dGeoData.shape[0], oDataTerrain.a2dGeoData.shape[1]])
            #-------------------------------------------------------------------------------------
            
            #-------------------------------------------------------------------------------------
            # Check data nature availability
            if oDataNature.oGeoData is None:
            
                #-------------------------------------------------------------------------------------
                # Checking terrain and vegtype finite values
                a2iNature[np.where(oDataTerrain.a2bGeoDataFinite==True)] = 1
                a2iNature[np.where(oDataVegType.a2bGeoDataFinite==False)] = 0
                
                # Set nodata value
                a2iNature[oDataTerrain.a2bGeoDataNan] = oDataTerrain.dNoData
                #-------------------------------------------------------------------------------------
                
            else:
                #-------------------------------------------------------------------------------------
                # Get data
                a2iNature = oDataNature.a2dGeoData
                # Set nodata value
                a2iNature[oDataNature.a2bGeoDataNan] = oDataNature.dNoData
                #-------------------------------------------------------------------------------------
            ...
            #-------------------------------------------------------------------------------------
            # Set boundaries with no data
            [a2iNature, oOut, oErr] = self.setNoDataBound(a2iNature, oDataTerrain.dNoData)
            #-------------------------------------------------------------------------------------
            ...
            # Get write method selection
            oApps_WriteMethod = getattr(oFileDrv.oFileWorkspace, 
                                      self.oVarsOUT['ASCII']['NatureType']['VarOp']['Op_Save']['Func'])
            oFileComp = self.oVarsOUT['ASCII']['NatureType']['VarOp']['Op_Save']
            # Write file
            oApps_WriteMethod(a2iNature, oDataVegType.a1oGeoInfo, oFileComp['Format'])
            # Close file
            oFileDrv.oFileWorkspace.closeFile()
            #-------------------------------------------------------------------------------------
            ...
            # Exit status without errors
            oOut = None; oErr = None;
            oLogStream.info( ' ------> Compute NatureType data ... OK')
            ...
            #-------------------------------------------------------------------------------------
            # Exit status with errors
            GetException(' -----> ERROR: Compute NatureType data ... FAILED',1,1)
            #-------------------------------------------------------------------------------------
    ...
    #-------------------------------------------------------------------------------------
    # Method to define nature map domain
    def computeAlertArea(self, oDataTerrain, oDataVegType, oDataAA):
        
        #-------------------------------------------------------------------------------------
        # Check section
        oLogStream.info( ' ------> Compute AlertArea data ... ')
        try:
            ...
            sFileNameOUT = Lib_Data_IO_Utils.defineFileName(join(
                                                           self.oPathInfo['DataStaticOutcome'], 
                                                           self.oVarsOUT['ASCII']['AlertArea']['VarSource']), 
                                                           {'domain' : self.oParamsInfo['DomainName']})
            #-------------------------------------------------------------------------------------
            
            #-------------------------------------------------------------------------------------
            # Initialize alertarea map
            a2iAA = np.zeros([oDataTerrain.a2dGeoData.shape[0], oDataTerrain.a2dGeoData.shape[1]])
            #-------------------------------------------------------------------------------------
            
            #-------------------------------------------------------------------------------------
            # Check data mask availability
            if oDataAA.oGeoData is None:
            
                #-------------------------------------------------------------------------------------
                # Checking terrain and vegtype finite values
                a2iAA[np.where(oDataTerrain.a2bGeoDataFinite==True)] = 1
                a2iAA[np.where(oDataVegType.a2bGeoDataFinite==False)] = 0
                
                # Set nodata value
                a2iAA[oDataTerrain.a2bGeoDataNan] = oDataTerrain.dNoData
                #-------------------------------------------------------------------------------------
                
            else:
                #-------------------------------------------------------------------------------------
                # Get data
                a2iAA = oDataAA.a2dGeoData
                # Set nodata value
                a2iAA[oDataAA.a2bGeoDataNan] = oDataAA.dNoData
                #-------------------------------------------------------------------------------------
            ...
            #-------------------------------------------------------------------------------------
            # Set boundaries with no data
            [a2iAA, oOut, oErr] = self.setNoDataBound(a2iAA, oDataTerrain.dNoData)
            #-------------------------------------------------------------------------------------
            ...
            # Get write method selection
            oApps_WriteMethod = getattr(oFileDrv.oFileWorkspace, 
                                      self.oVarsOUT['ASCII']['AlertArea']['VarOp']['Op_Save']['Func'])
            oFileComp = self.oVarsOUT['ASCII']['AlertArea']['VarOp']['Op_Save']
            # Write file
            oApps_WriteMethod(a2iAA, oDataVegType.a1oGeoInfo, oFileComp['Format'])
            # Close file
            oFileDrv.oFileWorkspace.closeFile()
            #-------------------------------------------------------------------------------------
            ...
            # Exit status without errors
            oOut = None; oErr = None;
            oLogStream.info( ' ------> Compute AlertArea data ... OK')
            ...
            #-------------------------------------------------------------------------------------
            # Exit status with errors
            GetException(' -----> ERROR: Compute AlertArea data ... FAILED',1,1)
            #-------------------------------------------------------------------------------------
