...
class FileCSV:
    ...
    def readFileData(self, oOpVarMethod=None):
        ...
        iCodeTest = -9999; iCodeStep = -9999; dValueFinal = 0; iCount = 0; iD = 0; iStep = 0;
...
class FileGrib:
    ...
    def readFileData(self, sFileNameGeo=None, oOpVarLoad=None, oOpVarMath=None):
        ...
        a1oTimeSteps = []; a2oData = {}; oTimeStepVarPrevious = None; iFMZero = 0; 
        ...
        for sFM in a1iFileMessages:
            ...
            if (oTimeStep_P2 - oTimeStep_P1).total_seconds() > iTimeForecastUnit:
                iTimeRatio = int((((oTimeStep_P2 - oTimeStep_P1).total_seconds())/iTimeForecastUnit)/iFM)
            ...
            if oTimeStep_P1 != oTimeAnalysis or oTimeStep_P2 != oTimeAnalysis:
                ...
                if iFM_TRI == 0:
                    ...
                    # Get data
                    #if iFM_Index == 1:
                    #    a2dVarDefAll = np.flipud(self.oFileData[iFM_Index].values)
                    #else:
                    #    a2dVarDefAll = np.flipud(self.oFileData[iFM_Index].values) - np.flipud(self.oFileData[iFM_Index-1].values)
                    
                    # Get data
                    a2dVarDef = np.flipud(self.oFileData[iFM_Index].values)

                    # Divide data in time ratio element(s)
                    a2dVarDef = a2dVarDef/float(iTimeRatio)
        ...
        for sTimeSave in a1oTimeSave:
            ...
            # info
            oLogStream.info(' ----> Save data :: VarName: ' + sVarName + ' TimeStep: ' + sTimeSave + ' ... ')
            ...
            # Info
            oLogStream.info(' ----> Save data :: VarName: ' + sVarName + ' TimeStep: ' + sTimeSave + ' ... OK ')
        #--------------------------------------------------------------------------------    
    ...
    def getFileAttrsCommon(self):
        ...
        for sAttrName in oAttrs:
            ...
            if isinstance(oAttrValue, basestring):
                ...
                GetException(' -----> WARNING: in reading file attribute(s). Attribute type not defined! (Attr: ' +  str(sAttrName) +')',2,1)
