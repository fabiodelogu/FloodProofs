...
def fft3d(z):
    ...
    for j in np.arange(-ns/2, ns/2):
        for i in np.arange(-ns/2, ns/2):
            k2 = np.sqrt(i*i+j*j)
            ik2 = np.floor(k2+1.5)  # Fabio: index issue in python 
            if ik2>1:
                
                ii = i + ns/2+1;
                jj = j + ns/2+1;
                value_fs = fs[ii-1,jj-1]; value_fx = fx[ik2-1]; value_nn = nn[ik2-1]+1;
...
def fitslope(fx, ii):
    ...
    s = np.zeros((nc,))
    for i in range(0, nc):
        ss = np.polyfit(np.log(ii), np.log(fx[ii,i]),1)
        s[i] = ss[0]
...
def metagauss(f):
    ...
    ph = np.zeros([ns,ns,nt])
    #for i in range(0,nt):
    #    ph[:,:,i] = np.random.randn(ns,ns)
...
def findslopes(dVarRAvg, dVarRMax, a1dVarRAvg, a1dVarSX, a1dVarST, a3dVarVM):
    
    try:
        # Compute index linked with mean value
        a1dVarRAvg_COM = np.abs(a1dVarRAvg - dVarRAvg);
        a1iVarRAvgMinIndex = np.where(a1dVarRAvg_COM == np.min(a1dVarRAvg_COM))[0];
        
        # Get S as function of VM
        a2dVarSVM = np.reshape( a3dVarVM[a1iVarRAvgMinIndex[0], :, :], [a3dVarVM.shape[1], a3dVarVM.shape[2]]);
        # Compute index linked with max value
        a2dVarRMax_COM = np.abs(a2dVarSVM - dVarRMax)
        [iVarSXIndex, iVarSTIndex] = np.where( a2dVarRMax_COM == np.min(a2dVarRMax_COM));
        
        # Get slope(s) X, Y and T
        dVarSX = a1dVarSX[iVarSXIndex]; 
        dVarSY = dVarSX
        dVarST = a1dVarST[iVarSTIndex];
        
    except:
        
        dVarSX = None; dVarSY= None; dVarST = None
    
    return dVarSX, dVarSY, dVarST
