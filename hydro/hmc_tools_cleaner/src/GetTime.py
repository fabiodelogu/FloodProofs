...
#-------------------------------------------------------------------------------------
# Method to compute timeperiod
def computeTimePeriod(sTime, iTimeRes, iTimeStepPast, iTimeStepFut=None):
    
    sTimeFormat = '%Y%m%d%H%M'
    oTime = datetime.datetime.strptime(sTime, sTimeFormat)
    
    if iTimeStepFut != None:
        oTimeTo = oTime + datetime.timedelta(seconds = iTimeRes*iTimeStepFut)
    else:
        oTimeTo = oTime
    ...
    oTimeFrom = oTimeTo - datetime.timedelta(seconds = iTimeRes*(iTimeStepPast-1))
    sTimeFrom = oTimeFrom.strftime('%Y%m%d%H%M')
    
    oTimeRes = datetime.timedelta(seconds = iTimeRes)
