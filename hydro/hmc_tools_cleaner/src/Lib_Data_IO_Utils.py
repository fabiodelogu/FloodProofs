...
def listObject(sPathName):
    
    a1oRootName = []; a1oFileName = []
    for sPathRoot, sPathFolder, sPathFiles in os.walk(sPathName):
        
        a1oRootName.append(sPathRoot)
        
        for sFile in sPathFiles:
            sFileName = os.path.join(sPathRoot, sFile)
            a1oFileName.append(sFileName)
    
    
    a1oRootName = sorted(a1oRootName)
    a1oFileName = sorted(a1oFileName)
    
    return a1oRootName, a1oFileName
...
#--------------------------------------------------------------------------------
# Method to get number from list
def getNumber(oList_IN , sNaN):   
    oList_OUT = []                                     
    for sX in oList_IN:
        
        if sX != sNaN:
            try:
                oList_OUT.append(float(sX))
            except ValueError:    
                pass
        else:
            oList_OUT.append(float(-9999.0))
