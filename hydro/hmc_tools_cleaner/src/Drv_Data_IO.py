...
class FileCSV:
    ...
    def openFile(self):
        ...
        try:
            ...
            elif 'wb' in self.sFileMode:
                oFile = csv.writer(oFileHandle, quoting=csv.QUOTE_NONNUMERIC)
            else:
                GetException(' -----> ERROR: in mode file (CSV I/O) Permitted values r w and wb',1,1)
                    
            self.oFileData = oFile
            self.oFileHandle = oFileHandle
    ...
    def readFileData(self, oOpVarMethod=None, bGetDataRaw=False):
        
        # Elaborated data raw
        if bGetDataRaw == False:
            ...
            # File object with elaborated data
            oFileObject = {}
            ...
            # File object with raw data
            oFileObject = {}
            oFileObject['Data'] = oDataRaw
...
class FileAscii:
    ...
    #--------------------------------------------------------------------------------
    # Write 2D data in ascii file (column array)
    def read2DVar(self, iSkipRows=0, sSplitRow='\t'):
        ...
        try:
            ...
            oTableData = oTableData[iSkipRows : -1]
            
            # Save file data in a dictionary
            oFileObject = {}
            oFileObject['Data'] = oTableData
            oFileObject['GeoX'] = None
            oFileObject['GeoY'] = None
            oFileObject['GeoZ'] = None
            oFileObject['Attributes'] = None
            ...
            # Exit status with error
            GetException(' -----> ERROR: in read2DVar function (ASCII I/O)',1,1)
    
    #--------------------------------------------------------------------------------
    
    #--------------------------------------------------------------------------------
    # Write 1D data in ascii file (column array)
    def read1DVar(self, skiprows=0):
        ...
        try:
            ...
            # Exit status with error
            GetException(' -----> ERROR: in read1DVar function (ASCII I/O)',1,1)
    
    #--------------------------------------------------------------------------------
    
    #--------------------------------------------------------------------------------
    # Write 1D data in ascii file (column array)
    def write2DVar(self, a2dDataValues, sDataFormat=None):
        ...
        try:
            ...
            if sDataFormat != None:
                sDataFormat = " ".join(re.findall("[a-zA-Z]+", sDataFormat))
            else:pass
            
            # Defining max number of digits before comma
            dVarDataMin = np.nanmin(np.unique(a2dDataValues))
            dVarDataMax = np.nanmax(np.unique(a2dDataValues))
            ...
            # Write array values
            np.savetxt(self.oFileData, a2dDataValues, fmt=sFmt,  delimiter=" ", newline='\n')

        except:
            # Exit status with error
            GetException(' -----> ERROR: in write2DVar function (ASCII I/O)',1,1)
    ...
    def readFileData(self, oDataComp=None):
        ...
        try:
            ...
            try:
                ...
                # Read data using load all field(s)
                ...
                oVarData = {}; a1sLine = [];
                self.oFileData.seek(0)
                while True:
                    
                    try:
                        sLine = self.oFileData.next()
                        sLine = sLine.replace(' ', '_');
                        a1sLine.append(sLine)
                    except:
                        break
...
class FileNetCDF:
    ...
    #--------------------------------------------------------------------------------
    # Method to read file in netCDF format
    def readFileData(self, sFileNameGeo=None, oOpVarLoad=None, oOpVarMath=None):
        ...
        # Get data
        oVarList = oFileData.variables
        oDimList = oFileData.dimensions
        oAttrList = oFileData.ncattrs()
        #--------------------------------------------------------------------------------
        
        #--------------------------------------------------------------------------------
        # File dimension(s)
        oVarsDims = {}
        for sDimElement in oDimList:
            
            sDimElement = sDimElement.encode('UTF-8')
            oVarsDims[sDimElement] = int(oVarList[sDimElement].__len__())
        #--------------------------------------------------------------------------------
        
        #--------------------------------------------------------------------------------  
        # File variable(s)
        oVarsData = {}; oGeoData = {}; oTimeData = {}
        for sVarName_IN in oVarList:
            
            # Get all data
            sVarName_IN = sVarName_IN.encode('UTF-8')
            oVarData_IN = oFileData.variables[sVarName_IN][:]
            
            if sVarName_IN == 'lon' or sVarName_IN == 'longitude':
                sVarName_OUT = 'GeoX'
                oVarData_OUT = oVarData_IN
                oGeoData[sVarName_OUT] = {}
                oGeoData[sVarName_OUT] = oVarData_OUT
                
            elif sVarName_IN == 'lat' or sVarName_IN == 'latitude':
                sVarName_OUT = 'GeoY'
                oVarData_OUT = oVarData_IN
                oGeoData[sVarName_OUT] = {}
                oGeoData[sVarName_OUT] = oVarData_OUT
                
            elif sVarName_IN == 'time' or sVarName_IN == 'Time':
                sVarName_OUT = 'Time'
                oVarData_OUT = oVarData_IN
                oTimeData[sVarName_OUT] = {}
                oTimeData[sVarName_OUT] = oVarData_OUT
                
            else:
                sVarName_OUT = sVarName_IN
                oVarData_OUT = oVarData_IN
                oVarsData[sVarName_OUT] = {}
                oVarsData[sVarName_OUT] = oVarData_OUT
        #--------------------------------------------------------------------------------  
        
        #--------------------------------------------------------------------------------   
        # File attribute(s)     
        oAttrData = {}
        for sAttrName in oAttrList:
            sAttrName = sAttrName.encode('UTF-8')
            sAttrValue = oFileData.getncattr(sAttrName)
            sAttrValue = sAttrValue.encode('UTF-8')
            oAttrData[sAttrName] = sAttrValue
        #--------------------------------------------------------------------------------
            
        #--------------------------------------------------------------------------------
        # File object
        oFileObject = {};
        oFileObject['Data'] = oVarsData
        oFileObject['GeoX'] = oGeoData['GeoX']
        oFileObject['GeoY'] = oGeoData['GeoY']
        oFileObject['GeoZ'] = None
        oFileObject['Attributes'] = oAttrData
        oFileObject['GeoInfo'] = {}
        oFileObject['Time'] = oTimeData['Time']
        #--------------------------------------------------------------------------------
