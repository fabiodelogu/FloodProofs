...
Name:          Cpl_Apps_RF_Model_ECMWF 0125
...
#-------------------------------------------------------------------------------------
# Class
class Cpl_Apps_RF_Model_ECMWF_0125:
        ...
        for sFileTypeIN in oVarsInfo_CHECK:
            ...
            for sVarName in oVarsInfo_CHECK[sFileTypeIN]:
                ...
                for sVarCompOUT in oVarCompOUT.values():
                    ...
                    #-------------------------------------------------------------------------------------
                    # BIN Check file
                    sFileCacheBIN_CHECK = os.path.join(sPathData_CHECK, 'RF_ECMWF-0125_FILEBIN_' + sTimeSave + '_' + sVarCompOUT + '.history')
                    ...
                    # NETCDF Check file
                    sFileCacheNC_CHECK = os.path.join(sPathData_CHECK, 'RF_ECMWF-0125_FILENC_' + sTimeSave + '_' + sVarCompOUT + '.history')
                ...
                if (oVarsSave[sVarName] is False):
                    ...
                    if sFileNameIN in a1oDataSelect:
                        ...
                        if a1oDataSelect[sFileNameIN]:
                            ...
                            iT = 0; iTCum = 6; iStepCum = 0; iK = 0;
                            a3dDataXYT_IN = np.zeros([a1oDataSelect[sFileNameIN]['GeoX'].shape[0], 
                                                      a1oDataSelect[sFileNameIN]['GeoX'].shape[1], 
                                                      iDataLen/iTCum])
                            a2dDataACC = np.zeros([a1oDataSelect[sFileNameIN]['GeoX'].shape[0], a1oDataSelect[sFileNameIN]['GeoX'].shape[1]]);
                            for iT in range(0,iDataLen):
                                ...
                                # Store data
                                a2dDataACC = a2dDataACC + a2dDataXY
                                iStepCum = iStepCum + 1;
                                
                                if iStepCum == iTCum:
                                    a3dDataXYT_IN[:,:,iK]  = np.flipud(a2dDataACC)
                                    iK = iK + 1
                                    iStepCum = 0
                                    a2dDataACC = np.zeros([a1oDataSelect[sFileNameIN]['GeoX'].shape[0], a1oDataSelect[sFileNameIN]['GeoX'].shape[1]]);
                                else:pass
                            ...
                            #sFile= 'ecmwf_data_server.mat'
                            #io.savemat(sPath+ sFile,mdict={'arr': a3dDataXYT_IN})
                            #-------------------------------------------------------------------------------------
                ...
                for sVarName in oVarData_Ensemble:
                    ...
                    if oVarsSave_OUT[sVarName] is False:
                        ...
                        if np.all(np.asarray(a1bFileBIN_CHECK) == True ):
                            # Create cache handle
                            sFileCacheBIN_CHECK = os.path.join(sPathData_CACHE, 'RF_ECMWF-0125_FILEBIN_' + sTimeSave + '_' + sVarName + '_' + sEnsembleN + '.history')
                        ...
                        if np.all(np.asarray(a1bFileNC_CHECK) == True ):
                            # Create hash handle
                            sFileCacheNC_CHECK = os.path.join(sPathData_CACHE, 'RF_ECMWF-0125_FILENC_' + sTimeSave + '_' + sVarName +'.history')
