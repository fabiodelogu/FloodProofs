...
class Drv_Model_RainFarm(object):
    ...
    def finalizer(a3dDataXYT_RF, a3dModelXYT_RF, a1oDataTime_RF,
        ...
        a3dDataXYT_REF = np.zeros([a2dGeoX_REF.shape[0], a2dGeoY_REF.shape[1],a3dModelXYT_RF.shape[2]])
        for iStep in range(0, a3dModelXYT_RF.shape[2]):
            ...
            a3dDataXYT_REF[:,:,iStep] = a2dDataXY_REF
        ...
        #sPath='/hydro/data/event_data/outcome/rainfarm/ecmwf0125/'
        #sFile= 'rf-scale.mat'
        #io.savemat(sPath+ sFile,mdict={'arr': a3dModelXYT_RF})
        #sFile= 'rf-interp.mat'
        #io.savemat(sPath+ sFile,mdict={'arr': a3dDataXYT_REF})
        #--------------------------------------------------------------------------------
...
#         path='/home/fabio/Desktop/Project_RegioneMarche/docs/rainfarm_nwp/'
#         data = {}
#         data['rain'] = X
#         sio.savemat(path+'rain.mat',data)
        
        # Conversion to obtain corrected factor
        ctsf = ctsf*modelstep;
        X = X/modelstep
        ...
        if cssf == 1 and ctsf == 1:
            ...
            pa = RFApps.agg_xyt(X[:, :, 0:nt], nx/cssf, ny/cssf, nt*modelstep/ctsf)
        ...
        if f is None:
            f = RFApps.initmetagauss(sx, st, nx*xscale, nt*modelstep)
        ...
        # Nonlinear transform
        r = np.exp(alfa * g[0:nx*xscale, 0:ny*yscale, 0:nt*modelstep]) 
        
        # We want the aggregated field to be the same as pa
        ga = RFApps.agg_xyt(r, nx/cssf, ny/cssf, nt*modelstep/ctsf)
        ca = pa/ga
        if  celle3_rainfarm is None:
            cai = RFApps.interpola_xyt(ca, nx*xscale, ny*yscale, nt*modelstep)
        else:
            cai = np.reshape(ca[celle3_rainfarm], (nx*xscale, ny*yscale, nt*modelstep),
                             order='F')
        
        # Define final rain field
        x = cai * r
        
        # Check volume disaggragation
        dVarX_ACC = np.sum(np.sum(X[:,:,0:nt]*modelstep*xscale*yscale));
        dVarx_ACC = np.sum(np.sum(x[:,:,0:nt*modelstep]));
        printMessage(' -------> Check VDis: XModel: '+str(dVarX_ACC)+' XDisag: '+str(dVarx_ACC))
