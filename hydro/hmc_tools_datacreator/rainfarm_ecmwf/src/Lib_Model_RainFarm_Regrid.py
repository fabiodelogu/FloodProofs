...
Name:          Lib_Model_RainFarm_Regrid
...
def getReferencesAndWeights(X1, Y1, X2, Y2):
    ...
    for i in range(0, iXYCount):
        ...
        for ii in range(0, iIJCount):
            ...
            iI = If[ii]
            iJ = Jf[ii]
            intersect_width  = np.nanmax([0, np.nanmin([X1[iI,iJ]+dx1/2, x2_r[i]]) - np.nanmax([X1[iI,iJ]-dx1/2, x2_l[i]])])                
            intersect_height = np.nanmax([0, np.nanmin([Y1[iI,iJ]+dy1/2, y2_u[i]]) - np.nanmax([Y1[iI,iJ]-dy1/2, y2_d[i]])])
...
if __name__ == '__main__':
    ...
    #    plt.close('all')
    #    plt.figure(); plt.imshow(B,interpolation='nearest'); plt.colorbar(); plt.axis('image')
    #    plt.show()
