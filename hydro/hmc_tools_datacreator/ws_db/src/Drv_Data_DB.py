...
class DB_RegMarche:
    ...
    def getData(self):
        ...
        #--------------------------------------------------------------------------------
        # Check variable db value(s)
        if self.sVarName == 'PP':
            oLogStream.info( ' ------> CHECK VARIABLE VALID RANGE ('+self.sVarName+') ... ')
            
            # Limit value
            dVarTrh = 200;

            oDataFinal = []; oDataSkipped = [];
            for oLine in a1oDBData:
    
                if oLine[3] > dVarTrh:
                    
                    oLogStream.info( ' -------> WARNING NO VALID DATA -- Value: '+str(oLine[3])+' -- REMOVE FROM DATA')
                    oLogStream.info( ' -------> WS DATA REMOVED: ' + str(oLine) )
                    oDataSkipped.append(oLine)
                else:
                    oDataFinal.append(oLine)

            a1oDBData = oDataFinal
            
            oLogStream.info( ' ------> CHECK VARIABLE VALID RANGE ('+self.sVarName+') ... OK')
        else:pass
        #--------------------------------------------------------------------------------
