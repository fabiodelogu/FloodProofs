...
#--------------------------------------------------------------------------------
# Method to fill nodata in interpolated var
def interpVarFillValue(a2dVar, oDataGeo, oDataFill=None):
    
    if oDataFill != None:
        
        a1iGeoDataNaN = oDataGeo.a1iGeoDataNaN
        a1iGeoDataFinite = oDataGeo.a1iGeoDataFinite
        
        a1dVar = a2dVar.ravel()
        
        a1iIndexVarNan = np.where(np.isnan(a1dVar[a1iGeoDataFinite]))[0]
        a1iIndexVarFill = a1iGeoDataFinite[a1iIndexVarNan]
        
        a1iIndexVarFinite = np.where(np.isfinite(a1dVar[a1iGeoDataFinite]))[0]
        a1iIndexVarMean = a1iGeoDataFinite[a1iIndexVarFinite]
        
        if len(a1iIndexVarFill) > 0:
    
            if isinstance(oDataFill, float):
                
                a1dVar[a1iIndexVarFill] = float(oDataFill)
                a2dVar = np.reshape(a1dVar, [ a2dVar.shape[0], a2dVar.shape[1] ])

            elif isinstance(oDataFill, int):
                
                a1dVar[a1iIndexVarFill] = float(oDataFill)
                a2dVar = np.reshape(a1dVar, [ a2dVar.shape[0], a2dVar.shape[1] ])
                
            elif isinstance(oDataFill, basestring):
                
                if oDataFill == 'meanVar':
                    
                    dVarMean = np.nanmean(a2dVar.ravel()[a1iIndexVarMean])
                    a1dVar[a1iIndexVarFill] = dVarMean
                    a2dVar = np.reshape(a1dVar, [ a2dVar.shape[0], a2dVar.shape[1] ])
...
def interpVarPointIDW(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar, 
    ...
    # Searching undefined values
    a1bDataVarIndexNaN = []; a1bDataVarIndexFinite = []; a1bDataVarIndexDataEx = []
    a1bDataVarIndexNaN = np.where(np.isnan(oDataVar) == True)
    
    if dDataExc != None:
        a1bDataVarIndexDataEx = np.where(oDataVar == dDataExc)
        a1bDataVarIndexNaN = tuple(set(list(a1bDataVarIndexNaN[0]) + list(a1bDataVarIndexDataEx[0])))
    else:
        a1bDataVarIndexDataEx = []
    ...
    if oGeoXVar != None and oGeoYVar != None:
        ...
        if a1iIndexSel is not None:
            ...
            sOption = []; # IDW = invdist=2.0
            sOption = ('-a invdist:power=2.0:smoothing=0.0:radius1=' + str(dRadiusX) + 
...
def interpVarPointMLR(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar, 
    ...
    # Get cols and rows
    iCols = oDataGeo.oGeoData.iCols
    iRows = oDataGeo.oGeoData.iRows
    #--------------------------------------------------------------------------------
    ...
    a1bVarIndexZero = []; a1dVarD = oDataVar
    a1bVarIndexZero = np.where(a1dVarD < 0.0)
    
    # Merge different nodata value(s)
    try:
        a1bVarIndexNaN = np.append(a1bVarIndexZero[0], a1bVarIndexNaN[0])
    except:
        a1bVarIndexNaN = a1bVarIndexNaN[0]
    #--------------------------------------------------------------------------------
    
    #--------------------------------------------------------------------------------
    # Get XY defined value(s)
    a1dVarD = np.delete(a1dVarD, a1bVarIndexNaN)
    oGeoXVar = np.delete(oGeoXVar, a1bVarIndexNaN)
    oGeoYVar = np.delete(oGeoYVar, a1bVarIndexNaN)
    #--------------------------------------------------------------------------------
    
    #--------------------------------------------------------------------------------
    # Count array non zero values
    if np.any(a1dVarD):
        iVarNZero = np.count_nonzero(a1dVarD)
    else:
        iVarNZero = 0;
    #--------------------------------------------------------------------------------
    ...
    #--------------------------------------------------------------------------------
    # Check no zeros data
    if np.any(a1dVarD) and iVarNZero > 1:
        ...
        a2dVarFinal[a2dVarFinal<1.0] = 0.0
        ...
        #sPath='/hydro/hmc_tools_datacreator/ws_db/'
        #sFile= 'interp-server.mat'
        #io.savemat(sPath+ sFile, 
        #           mdict={'w': a2dVarW, 'final': a2dVarFinal, 'data': a2dVarMap, 'residual': oDataInterp['Var_1']})
        
        #plt.figure(1)
        #plt.imshow(a2dVarMap); plt.colorbar();plt.clim(0, 500)
        #plt.figure(2)
        #plt.imshow(oDataInterp['Var_1']); plt.colorbar(); plt.clim(0, 500)
        #plt.figure(3)
        #plt.imshow(a2dVarFinal); plt.colorbar(); plt.clim(0, 500)
        #plt.figure(4)
        ...
        #--------------------------------------------------------------------------------
        # Exit with nodata values
        a2dVarFinal = np.zeros((iRows, iCols)); 
        a2dVarW = np.zeros((iRows, iCols)); 
        #--------------------------------------------------------------------------------
...
def interpVarGridNN(oDataGeo, oDataVar, oGeoXVar, oGeoYVar, oGeoZVar=None, dNoData=np.nan, a2iIndexVar=None):
    ...
    if a2iIndexVar is None:
        ...
        a2dDataVarRegrid = griddata((a2dGeoXVar.ravel(), a2dGeoYVar.ravel()), a2dDataVar.ravel(),
                                              (a2dGeoXRef, a2dGeoYRef), method='nearest', fill_value = dNoData)
          
    elif a2iIndexVar is not None:
        
            a1dDataVarRegrid = a2dDataVar.ravel()[a2iIndexVar.ravel()]
