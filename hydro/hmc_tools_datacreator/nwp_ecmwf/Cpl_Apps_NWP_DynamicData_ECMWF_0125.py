...
Name:          Cpl_Apps_NWP_DynamicData_ECMWF_0125
...
#-------------------------------------------------------------------------------------
# Class
class Cpl_Apps_NWP_DynamicData_ECMWF_0125:
        ...
        for sFileTypeIN in oVarsInfo_CHECK:
            ...
            for sVarName in oVarsInfo_CHECK[sFileTypeIN]:
                ...
                for sVarCompOUT in oVarCompOUT.values():
                    ...
                    #-------------------------------------------------------------------------------------
                    # BIN Check file
                    sFileCacheBIN_CHECK = os.path.join(sPathData_CHECK, 'NWP_ECMWF-0125_FILEBIN_' + sTimeSave + '_' + sVarCompOUT + '.history')
                    ...
                    # NETCDF Check file
                    sFileCacheNC_CHECK = os.path.join(sPathData_CHECK, 'NWP_ECMWF-0125_FILENC_' + sTimeSave + '_' + sVarCompOUT + '.history')
        ...
        if np.any(a2bSaveVar_CHECK.values()) == False:
            for sFileCache in oFileCache_CHECK:
                ...
                if os.path.exists(sFileCache):
                    ...
                    elif isinstance(oFileSaved, list):
                        oFileSaved = oFileSaved[0]
                    
                    for sFileSaved in oFileSaved:
            ...
            for sVarName in self.oVarData:
                ...
                if oVarsSave_OUT[sVarName] is False:
                    ...
                    if np.all(np.asarray(a1bFileBIN_CHECK) == True ):
                        # Create cache handle
                        sFileCacheBIN_CHECK = os.path.join(sPathData_CACHE, 'NWP_ECMWF-0125_FILEBIN_' + sTimeSave + '_' + sVarName + '.history')
                    ...
                    if np.all(np.asarray(a1bFileNC_CHECK) == True ):
                        # Create hash handle
                        sFileCacheNC_CHECK = os.path.join(sPathData_CACHE, 'NWP_ECMWF-0125_FILENC_' + sTimeSave + '_' + sVarName +'.history')
