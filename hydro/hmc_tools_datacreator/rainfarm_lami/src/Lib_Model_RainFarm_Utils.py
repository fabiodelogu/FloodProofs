...
def computeGrid(a2dGeoX_IN, a2dGeoY_IN, 
    ...
    iGeoX_Dim = a2dGeoX_IN.shape[0]; iGeoY_Dim = a2dGeoY_IN.shape[1];
    dGeoY_IN_MinTEST = a2dGeoY_IN[-1, 0]; dGeoY_IN_MaxTEST = a2dGeoY_IN[0, 0];
    
    printMessage( ' -------> Checking SOUTH_NORTH GEOY_IN orientation  (GEOY_IN Min: '+str(dGeoY_IN_MinTEST)+' GEOY_IN Max: ' + str(dGeoY_IN_MaxTEST)+')')
    if (dGeoY_IN_MinTEST > dGeoY_IN_MaxTEST):
        printMessage( ' -------> Changing grid GEOY_IN orientation ... SOUTH_NORTH BAD ORIENTATION!')
        a2dGeoY_IN = np.flipud(a2dGeoY_IN)
        printMessage( ' -------> Changing grid GEOY_IN orientation ... OK')
    else:
        printMessage( ' -------> Changing grid GEOY_IN orientation ... SOUTH_NORTH CORRECT ORIENTATION')
        printMessage( ' -------> Changing grid GEOY_IN orientation ... SKIPPED')
    #--------------------------------------------------------------------------------
...
def computeTimeSteps(sTimeFrom_IN, sTimeTo_IN, iTimeStep_IN, iTimeRatio_OUT):
    ...
    oTimeDiff_IN = oTimeTo_IN - oTimeFrom_IN; iTimeDiff_IN = oTimeDiff_IN.total_seconds() + 3600
    # Compute time delta (IN)
    iTimeDelta_IN = int(iTimeDiff_IN)/int(iTimeStep_IN) # step -1
    ...
    oTimeStep_IN = oTimeFrom_IN - datetime.timedelta(seconds = iTimeDelta_OUT)
