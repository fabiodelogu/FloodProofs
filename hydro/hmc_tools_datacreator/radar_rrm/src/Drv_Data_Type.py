...
class Drv_Data_Type:
    ...
    def getVarPoint(self, oFileWorkspace=None, oVarOp=None, dNoData=None, sTime=None, sFileNameGeo=None, bGetDataRaw=False):
        
        # Check file workspace availability
        oPointObject = {}
        if oFileWorkspace:
            
            # Extract data from grid file (using a common method readFileData) --> different flag for raw data
            if bGetDataRaw == False:
                oFileData = oFileWorkspace.readFileData(oVarOp)
            elif bGetDataRaw == True:
                oFileData = oFileWorkspace.readFileData(oVarOp, bGetDataRaw)
            else:
                GetException(' -----> ERROR: get var point problem with flag for data raw! Check your call!', 1, 1)
            ...
            if oFileData:
                ...
                if self.bIndex:
                    ...
                    GetException(' -----> WARNING: no TIME information available! Save only file extracted value(s)!', 2, 1)

                    oPointObject = {};
                    oPointObject['Data'] = {}
                    oPointObject['Data'] [sTime] = {}
                    oPointObject['Data'] [sTime] [sVarName]= {}
                    oPointObject['Data'] [sTime] [sVarName] = oFileData['Data']
                    oPointObject['Attributes'] = None
                    oPointObject['GeoX'] = None
                    oPointObject['GeoY'] = None
                    oPointObject['GeoZ'] = None
                    oPointObject['Time'] = sTime
                    oPointObject['GeoInfo'] = None
            else:
