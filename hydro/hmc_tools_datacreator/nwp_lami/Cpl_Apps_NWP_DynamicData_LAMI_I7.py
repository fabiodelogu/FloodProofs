...
Name:          Cpl_Apps_NWP_DynamicData_LAMI_I7
...
#-------------------------------------------------------------------------------------
# Class
class Cpl_Apps_NWP_DynamicData_LAMI_I7:
        ...
        for sFileTypeIN in oVarsInfo_CHECK:
            ...
            for sVarName in oVarsInfo_CHECK[sFileTypeIN]:
                ...
                for sVarCompOUT in oVarCompOUT.values():
                    ...
                    #-------------------------------------------------------------------------------------
                    # BIN Check file
                    sFileCacheBIN_CHECK = os.path.join(sPathData_CHECK, 'NWP_LAMI-I7_FILEBIN_' + sTimeSave + '_' + sVarCompOUT + '.history')
                    ...
                    # NETCDF Check file
                    sFileCacheNC_CHECK = os.path.join(sPathData_CHECK, 'NWP_LAMI-I7_FILENC_' + sTimeSave + '_' + sVarCompOUT + '.history')
            ...
            for sVarName in self.oVarData:
                ...
                if oVarsSave_OUT[sVarName] is False:
                    ...
                    if np.all(np.asarray(a1bFileBIN_CHECK) == True ):
                        # Create cache handle
                        sFileCacheBIN_CHECK = os.path.join(sPathData_CACHE, 'NWP_LAMI-I7_FILEBIN_' + sTimeSave + '_' + sVarName + '.history')
                    ...
                    if np.all(np.asarray(a1bFileNC_CHECK) == True ):
                        # Create hash handle
                        sFileCacheNC_CHECK = os.path.join(sPathData_CACHE, 'NWP_LAMI-I7_FILENC_' + sTimeSave + '_' + sVarName +'.history')
