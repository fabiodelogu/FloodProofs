...
SUBROUTINE SurfaceRoutingDamLaghi(iRows,iCols,dDintegr,dThydro,a2dQtmp)
implicit none
...
REAL*8 dH1,dH2 !Livelli nei 2 rami della confluenza dei fiumi
REAL*8 dH1f,dQt !H1 fittizia
REAL*8 matnan(iRows,iCols),nansum
...
DO iP=1,iNprese
	...
	j=a2dXYPresa(iP,1)
	!transform in in mm
	dDH=a1dIdroPrese(iP,iTtemp+1)/(a2dAreaCell(i,j))*dDth*1000
	if(1.eq.1)then
		ii=int((a2iPun(i,j)-1)/3)-1
		jj=a2iPun(i,j)-5-3*ii
		iii=i+ii
...
DO iD=1,iNjoin
	...
	if(dH1.lt.dH2.and.dH2.gt.0)then
		...
		if(dH1f/dH2.lt.a1dThreshLiv(iD))then
			...
			!Master
			a2dRouting(ii,jj)=a2dRouting(ii,jj)+a2dRouting(i,j)*(1-dH1f/dH2) !Routing dove immetto la derivazione del Master
			a2dRouting(i,j)=a2dRouting(i,j)*dH1f/dH2 !Routing che rimane in alveo
			!Immissario
			a2dRouting(ii,jj)=a2dRouting(ii,jj)+a2dRouting(ia,ja)*(1-dH1f/dH2) !Routing dove immetto la derivazione dell'immissario
			a2dRouting(ia,ja)=a2dRouting(ia,ja)*dH1f/dH2 !Routing che rimane in alveo
		endif
	endif
