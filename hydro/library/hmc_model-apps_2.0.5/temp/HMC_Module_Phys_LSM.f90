...
contains
    ...
    subroutine HMC_Phys_LSM_Cpl(iID, iRows, iCols, sTime)
        ...
        where( a2dVarDEM.gt.0.0 )
            ...
            ! Air pressure using DEM data (default)
            !a2dVarPa = 101.3*((293 - 0.0065*a2dVarDEM)/293)**5.26	![kPa]
	...
	! Net Radiation [W/m^2] --> sigma [W/m^2 K^4], EpsA [%], EpsS [%], albedo [-], Ta [K], LST [K], K [W/m^2]
        ...
        endwhere
        ! Where LST difference is too big then put old LST
        where(a2dVarLST_Diff.gt.dVarLST_DiffMax*iDtDataForcing/3600.0)
            a2dVarLSTUpd = a2dVarLSTPStep
        endwhere
        ! Set LST maximum
        where(a2dVarLSTUpd.gt.273.15 + 70.0)
            a2dVarLSTUpd = 273.15 + 70.0
        endwhere
        !-----------------------------------------------------------------------------------------
