...
contains
    ...
    subroutine HMC_Data_Restart_Gridded_Cpl( iID, sTime, &
        ...
        real(kind = 4), dimension(iRowsEnd - iRowsStart + 1, iColsEnd - iColsStart + 1) :: a2dVarWTableUpd                                                                                   a2dVarVWtUpd
    ...
    subroutine HMC_Data_Restart_Gridded_NC(iID, &
        ...
        a1bVarCheck = .false.; bCheckRestart = .false.; 
        !------------------------------------------------------------------------------------------
    ...
    subroutine HMC_Data_Restart_Gridded_Binary(iID, &
        ...
        character(len = 700)                    :: sFileNameData_Restart, sCommandUnzipFile
        character(len = 256)                    :: sVarName
        integer(kind = 4), intent(in)           :: iRows, iCols
        integer(kind = 4), intent(in)           :: iDaySteps, iTMarkedSteps
        integer(kind = 4), intent(in)           :: iFlagSnow
        ...
        !------------------------------------------------------------------------------------------
        ! VTot  (example: VTot_201405010000.bin.gz)
        iScaleFactor = 10000
        sFileNameData_Restart = trim(sPathData_Restart)//"VTot_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        ...
        if ( .not. bFileExist ) then
            call mprintf(.true., iWARN, ' ATTENTION:  Problem opening uncompressed binary file: '// &
                         trim(sFileNameData_Restart)//'.gz --> Undefined restart data values' )
            a2dVar = -9999.0
            a1bVarCheck(1) = .false.
        else
            ! Unzip file
            call HMC_Tools_Generic_UnzipFile(sCommandUnzipFile, &
                                             trim(sFileNameData_Restart)//'.gz', &
                                             trim(sFileNameData_Restart), .true.)
            ! Read binary data
            a2dVar = -9999.0
        ...
        !------------------------------------------------------------------------------------------
        ! VRet  (example: VRet_201405010000.bin.gz)
        iScaleFactor = 10000
        sFileNameData_Restart = trim(sPathData_Restart)//"VRet_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        call mprintf(.true., iINFO_Extra, ' Get filename: '//trim(sFileNameData_Restart) )
        ...
        !------------------------------------------------------------------------------------------
        ! HydroLevel  (example: HydroLevel_201405010000.bin.gz)
        iScaleFactor = 100000
        sFileNameData_Restart = trim(sPathData_Restart)//"HydroLevel_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        call mprintf(.true., iINFO_Extra, ' Get filename: '//trim(sFileNameData_Restart) )
        ...
        !------------------------------------------------------------------------------------------
        ! Routing  (example: Routing_201405010000.bin.gz)
        iScaleFactor = 100000
        sFileNameData_Restart = trim(sPathData_Restart)//"Routing_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        call mprintf(.true., iINFO_Extra, ' Get filename: '//trim(sFileNameData_Restart) )
        ...
        !------------------------------------------------------------------------------------------
        ! WTLevel  (example: WTLevel_201405010000.bin.gz)
        iScaleFactor = 10000
        sFileNameData_Restart = trim(sPathData_Restart)//"WTLevel_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        call mprintf(.true., iINFO_Extra, ' Get filename: '//trim(sFileNameData_Restart) )
        ...
        !------------------------------------------------------------------------------------------
        ! LST  (example: LST_201405010000.bin.gz)
        iScaleFactor = 10000
        sFileNameData_Restart = trim(sPathData_Restart)//"LST_"// &
            sTime(1:4)//sTime(6:7)//sTime(9:10)// & 
            sTime(12:13)//sTime(15:16)// &
            ".bin"  
        call mprintf(.true., iINFO_Extra, ' Get filename: '//trim(sFileNameData_Restart) )
        ...
        !------------------------------------------------------------------------------------------
        ! T24  (example: Tmk_201405010000.bin.gz)
        iScaleFactor = 10000
