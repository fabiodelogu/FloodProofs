...
contains
    ...
    subroutine HMC_Data_Forcing_Gridded_Cpl( iID, sTime, &
        ...
        if (oHMC_Vars(iID)%iTime .lt. oHMC_Namelist(iID)%iNTime) then
            ...
            a2dVarRainL = check2Dvar(a2dVarRainL,               oHMC_Vars(iID)%a2iMask,     0.0,    850.0   )
            a2dVarTaL = check2Dvar(a2dVarTaL,                   oHMC_Vars(iID)%a2iMask,     -70.0,  60.0    )    
            a2dVarIncRadL = check2Dvar(a2dVarIncRadL,           oHMC_Vars(iID)%a2iMask,     0.0,    1412.0  ) 
            a2dVarWindL = check2Dvar(a2dVarWindL,               oHMC_Vars(iID)%a2iMask,     0.0,    80.0    ) 
            a2dVarRelHumL = check2Dvar(a2dVarRelHumL,           oHMC_Vars(iID)%a2iMask,     0.0,    100.0   )
            a2dVarSnowHeightL = check2Dvar(a2dVarSnowHeightL,   oHMC_Vars(iID)%a2iMask,     0.0,    1000.0  ) 
            a2dVarSnowKernelL = check2Dvar(a2dVarSnowKernelL,   oHMC_Vars(iID)%a2iMask,     0.0,    1.0     )
        ...
        ! Air pressure
        if ( .not. all(a2dVarPaL.eq.-9999.0) ) then
