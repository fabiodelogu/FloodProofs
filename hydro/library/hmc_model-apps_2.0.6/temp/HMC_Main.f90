...
! Adding in: Project-->Properties-->Linker
!   netcdff.a and netcdff.so (note double f for fortran libraries)
! Adding in: Source Files-->Properties-->Additional Options:
!   -I/usr/local/netcdf-4.1.2/include/ -I/usr/local/hdf5-1.8.6/include/ -L=/usr/local/netcdf-4.1.2/lib -lnetcdff -lnetcdf -L=/usr/local/hdf5-1.8.6/lib -lhdf5_hl -lhdf5
!
! DEBUG USING GNUPLOT (gnufor2):
! call surf(dble(VAR),pm3d='pm3d implicit map', palette='rgbformulae 31, -11, 32')
! DEBUG USING FORTRAN+MATLAB: [ var*8 = dble(var*4) ]
...
program HMC_Main
    ...
    TimeLoop : do iTime = 1, iETime
        ...
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dRain), iRowsL, iColsL, 1)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dTa), iRowsL, iColsL, 2)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dK), iRowsL, iColsL, 3)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dW), iRowsL, iColsL, 4)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dRHum), iRowsL, iColsL, 5)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dVTot), iRowsL, iColsL, 6)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dVRet), iRowsL, iColsL, 7)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dHydro), iRowsL, iColsL, 8)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dRouting), iRowsL, iColsL, 9)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dWTable), iRowsL, iColsL, 10)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dLST), iRowsL, iColsL, 11)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a2dFlowDeep), iRowsL, iColsL, 12)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a3dTaKMarked(:,:,1)), iRowsL, iColsL, 13)
        !call debug_2dVar(dble(oHMC_Vars(iID)%a3dTaK24(:,:,1)), iRowsL, iColsL, 14)
        !write(*,*)
