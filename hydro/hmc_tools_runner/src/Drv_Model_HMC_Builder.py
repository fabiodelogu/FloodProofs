...
class Drv_Model_HMC_Builder:
    ...
    def defineRunName(self):
        ...
        sFileNameExec = self.oDataInfo.oInfoSettings.oParamsInfo['FileExecName']
        
        try:
            sPathRun = self.oDataInfo.oInfoSettings.oPathInfo['RunRaw']
            sPathCache = self.oDataInfo.oInfoSettings.oPathInfo['DataCacheRaw']
            sPathTemp = self.oDataInfo.oInfoSettings.oPathInfo['DataTempRaw']
            sRunCommand = self.oDataInfo.oInfoSettings.oPathInfo['RunCommandRaw']
            sPathLib = self.oDataInfo.oInfoSettings.oPathInfo['LibraryRaw']
            sFileNameExec = self.oDataInfo.oInfoSettings.oParamsInfo['FileExecNameRaw'] 
        except:
            self.oDataInfo.oInfoSettings.oPathInfo['RunRaw'] = sPathRun
            self.oDataInfo.oInfoSettings.oPathInfo['DataCacheRaw'] = sPathCache
            self.oDataInfo.oInfoSettings.oPathInfo['DataTempRaw'] = sPathTemp
            self.oDataInfo.oInfoSettings.oPathInfo['RunCommandRaw'] = sRunCommand
            self.oDataInfo.oInfoSettings.oPathInfo['LibraryRaw'] = sPathLib
            self.oDataInfo.oInfoSettings.oParamsInfo['FileExecNameRaw'] = sFileNameExec
        #-------------------------------------------------------------------------------------
    ...
    def selectTimeRestart(self):
        ...
        if iTimeRestartFlag == 1:
            ...
            if oTimeRestartSettings['TimeDay'] and oTimeRestartSettings['TimeHH']:
                ...
                if oTimeRestart > oTimeFrom:
                    ...
                    oTimeFrom = oTimeRestart + datetime.timedelta(seconds = int(iTimeStep))
                else:
                    pass
        ...
        self.oDataTime.oTimeRestartUpd = oTimeRestart
    ...
    #-------------------------------------------------------------------------------------
    # Method to clear run workspace (folders and so on)
    def clearRunWorkspace(self):
        ...
        # Get time from
        oTimeFrom = self.oDataTime.oTimeFrom
        #-------------------------------------------------------------------------------------
        
        #-------------------------------------------------------------------------------------
        # GRIDDED OUTCOME 
        # Get generic path and create run path
        sFilePathOG = self.oDataInfo.oInfoVarDynamic.oDataOutputDynamic['Gridded']['FilePath']
        sFilePathOG = Lib_Data_IO_Utils.defineString(sFilePathOG, {'$RUN' : sRunName})
        sFilePathOG = Lib_Data_IO_Utils.defineString(sFilePathOG, {'$TYPE' : sEnsembleName})
        sFilePathOG = sFilePathOG.split('$yyyy')[0]; 
        
#         # Delete old folder(s)
#         Lib_Data_IO_Utils.deleteFolderOld(sFilePathOG, oTimeFrom)
#         
#         # Delete empty folder(s)
#         Lib_Data_IO_Utils.deleteFolderEmpty(sFilePathOG)
        
        sFilePathOG = sFilePathOG.replace('//','/')
        if os.path.exists(sFilePathOG):
            shutil.rmtree(sFilePathOG)
            Lib_Data_IO_Utils.createFolder(sFilePathOG, '$yyyy')
        else:
            Lib_Data_IO_Utils.createFolder(sFilePathOG, '$yyyy')
        
        
#         # Search folder with a defined tag and delete it
#         a1oFilePathOG = Lib_Data_IO_Utils.searchFolder(sFilePathOG, sEnsembleName)
#         for sFilePathOG_Check in a1oFilePathOG: 
#         
#             sFilePathOG_Check = sFilePathOG_Check.replace('//','/')
#         
#             if os.path.exists(sFilePathOG_Check):
#                 try:
#                     shutil.rmtree(sFilePathOG_Check)
#                     Lib_Data_IO_Utils.createFolder(sFilePathOG_Check, '$yyyy')
#                 except:
#                     GetException(' -----> WARNING: deleting folder ' +sFilePathOG_Check+ ' failed! Check your permission(s) and file system!',1,1)  
# 
#             else: Lib_Data_IO_Utils.createFolder(sFilePathOG_Check, '$yyyy')
        #-------------------------------------------------------------------------------------
        
        #-------------------------------------------------------------------------------------
        # POINT OUTCOME 
        # Get generic path and create run path
        sFilePathOP = self.oDataInfo.oInfoVarDynamic.oDataOutputDynamic['Point']['FilePath']
        sFilePathOP = Lib_Data_IO_Utils.defineString(sFilePathOP, {'$RUN' : sRunName})
        sFilePathOP = Lib_Data_IO_Utils.defineString(sFilePathOP, {'$TYPE' : sEnsembleName})
        sFilePathOP = sFilePathOP.split('$yyyy')[0]; 
        
#         # Delete old folder(s)
#         Lib_Data_IO_Utils.deleteFolderOld(sFilePathOP, oTimeFrom)
#         
#         # Delete empty folder(s) 
#         Lib_Data_IO_Utils.deleteFolderEmpty(sFilePathOP)
        
        if sFilePathOP != sFilePathOG:
        
            sFilePathOP = sFilePathOP.replace('//','/')
            if os.path.exists(sFilePathOP):
                shutil.rmtree(sFilePathOP)
                Lib_Data_IO_Utils.createFolder(sFilePathOP, '$yyyy')
            else:
                Lib_Data_IO_Utils.createFolder(sFilePathOP, '$yyyy')
        else: pass
        
#         # Search folder with a defined tag and delete it
#         a1oFilePathOP = Lib_Data_IO_Utils.searchFolder(sFilePathOP, sEnsembleName)
#         for sFilePathOP_Check in a1oFilePathOP: 
#         
#             sFilePathOP_Check = sFilePathOP_Check.replace('//','/')
#         
#             if os.path.exists(sFilePathOP_Check):
#                 try:
#                     shutil.rmtree(sFilePathOP_Check)
#                     Lib_Data_IO_Utils.createFolder(sFilePathOP_Check, '$yyyy')
#                 except:
#                     GetException(' -----> WARNING: deleting folder ' +sFilePathOP_Check+ ' failed! Check your permission(s) and file system!',1,1)  
# 
#             else: Lib_Data_IO_Utils.createFolder(sFilePathOP_Check, '$yyyy')
        #-------------------------------------------------------------------------------------
        
        #-------------------------------------------------------------------------------------
        # GRIDDED STATE
        # Get generic path and create run path
        sFileNameSG = self.oDataInfo.oInfoVarDynamic.oDataStateDynamic['Gridded']['FilePath']
        sFileNameSG = Lib_Data_IO_Utils.defineString(sFileNameSG, {'$RUN' : sRunName})
        sFileNameSG = Lib_Data_IO_Utils.defineString(sFileNameSG, {'$TYPE' : sEnsembleName})
        sFileNameSG = sFileNameSG.split('$yyyy')[0]; 
        
#         # Delete old folder(s)
#         Lib_Data_IO_Utils.deleteFolderOld(sFileNameSG, oTimeFrom)
#         
#         # Delete empty folder(s) 
#         Lib_Data_IO_Utils.deleteFolderEmpty(sFileNameSG)
        
        sFileNameSG = sFileNameSG.replace('//','/')
        if os.path.exists(sFileNameSG):
            shutil.rmtree(sFileNameSG)
            Lib_Data_IO_Utils.createFolder(sFileNameSG, '$yyyy')
        else:
            Lib_Data_IO_Utils.createFolder(sFileNameSG, '$yyyy')
        
#         # Search folder with a defined tag and delete it
#         a1oFilePathSG = Lib_Data_IO_Utils.searchFolder(sFileNameSG, sEnsembleName)
#         for sFilePathSG_Check in a1oFilePathSG: 
#         
#             sFilePathSG_Check = sFilePathSG_Check.replace('//','/')
#         
#             if os.path.exists(sFilePathSG_Check):
#                 try:
#                     shutil.rmtree(sFilePathSG_Check)
#                     Lib_Data_IO_Utils.createFolder(sFilePathSG_Check, '$yyyy')
#                 except:
#                     GetException(' -----> WARNING: deleting folder ' +sFilePathSG_Check+ ' failed! Check your permission(s) and file system!',1,1)  
# 
#             else: Lib_Data_IO_Utils.createFolder(sFilePathSG_Check, '$yyyy')
        #-------------------------------------------------------------------------------------
        
        #-------------------------------------------------------------------------------------
        # POINT STATE
        # Get generic path and create run path
        sFileNameSP = self.oDataInfo.oInfoVarDynamic.oDataStateDynamic['Gridded']['FilePath']
        sFileNameSP = Lib_Data_IO_Utils.defineString(sFileNameSP, {'$RUN' : sRunName})
        sFileNameSP = Lib_Data_IO_Utils.defineString(sFileNameSP, {'$TYPE' : sEnsembleName})
        sFileNameSP = sFileNameSP.split('$yyyy')[0]; sFileNameSP = sFileNameSP.replace('//','/')
        
#         # Delete old folder(s)
#         Lib_Data_IO_Utils.deleteFolderOld(sFileNameSP, oTimeFrom)
#         
#         # Delete empty folder(s) 
#         Lib_Data_IO_Utils.deleteFolderEmpty(sFileNameSP)
        
        if sFileNameSP != sFileNameSG:
        
            sFileNameSP = sFileNameSP.replace('//','/')
            if os.path.exists(sFileNameSP):
                shutil.rmtree(sFileNameSP)
                Lib_Data_IO_Utils.createFolder(sFileNameSP, '$yyyy')
            else:
                Lib_Data_IO_Utils.createFolder(sFileNameSP, '$yyyy')
...
#         # Search folder with a defined tag and delete it
#         a1oFilePathSP = Lib_Data_IO_Utils.searchFolder(sFileNameSP, sEnsembleName)
#         for sFilePathSP_Check in a1oFilePathSP: 
#         
#             sFilePathSP_Check = sFilePathSP_Check.replace('//','/')
#         
#             if os.path.exists(sFilePathSP_Check):
#                 try:
#                     shutil.rmtree(sFilePathSP_Check)
#                     Lib_Data_IO_Utils.createFolder(sFilePathSP_Check, '$yyyy')
#                 except:
#                     GetException(' -----> WARNING: deleting folder ' +sFilePathSP_Check+ ' failed! Check your permission(s) and file system!',1,1)  
# 
#             else: Lib_Data_IO_Utils.createFolder(sFilePathSP_Check, '$yyyy')
    ...
    def getFileStaticInfo(self):
        ...
        # Get history file
        try:
            self.oFileHistory = getFileHistory(self.sFileHistory_NOW)
        except:
            GetException(' -----> WARNING: history file not found in file system! Check your settings!',2,1)
            self.oFileHistory = None
        #------------------------------------------------------------------------------------- 
        
        #------------------------------------------------------------------------------------- 
        # Check file history availability
        if (self.oFileHistory):
            ...
            #-------------------------------------------------------------------------------------
            # Info exit
            oLogStream.info( ' ====> GET AND UPDATE FILE INFO ... FAILED')
            GetException(' -----> WARNING: history file not found or null in file system! Data N/A!',2,1)
            #-------------------------------------------------------------------------------------
    ...
    def getFileDynamicGridded(self):
        ...
        # Get static data X,Y and Z
        a2dGeoX = self.oDataStatic['GeoX']
        a2dGeoY = self.oDataStatic['GeoY']
        a2dGeoZ = self.oDataStatic['GeoZ']
        ...
        iTimePeriodObs = self.oDataInfo.oInfoSettings.oParamsInfo['TimePeriodObs']
        iTimePeriodFor = self.oDataInfo.oInfoSettings.oParamsInfo['TimePeriodFor']
        ...
        a1sVarNameOBS_SELECT = []; a1sVarNameFOR_SELECT = []
                ...
                for sVarName_CHECK in oVarsDynamic:        
                    ...
                    for sTime_CHECK in a1oTime_CHECK:
                        ...
                        if bFileExist_CHECK == True:
                            sFileName_SELECT = sFileName_CHECK; sVarName_SELECT = sVarName_CHECK; sTime_SELECT = sTime_CHECK
                            oLogStream.info(' -------> Select File: ' + sFileName_SELECT + ' ... OK ')
                            break
                        else:
                            # Info
                            GetException(' -------> WARNING: Select File: ' + sFileName_CHECK + ' ... SKIPPED --- FILE NOT FOUND --- STEP SKIPPED IN MODEL RUN !' ,2,1)
                            sFileName_SELECT = sFileName_CHECK; sVarName_SELECT = sVarName_CHECK; sTime_SELECT = sTime_CHECK
                    #-------------------------------------------------------------------------------------
                    ...
                    if bFileExist_CHECK is True:
                        ...
                        # Get data 
                        [oFileData_SELECT, oFileVarStatus_SELECT] = oFileDrv_SELECT.getDynamicData(
                                                                       sVarFileVar_CHECK, sTime_IN, iVarDims)
                        ...
                        if not (oData_SELECT == None and oTime_SELECT == None):
                            ...
                            for iTime_SELECT, sTime_SELECT in enumerate(oTime_SELECT):
                                ...
                                if oFileVarStatus_SELECT:
                                    ...
                                    if sVarType_IN == 'OBS':
                                        a1sVarNameOBS_SELECT.append(sVarName_SELECT)
                                    elif sVarType_IN == 'FOR':
                                        a1sVarNameFOR_SELECT.append(sVarName_SELECT)

                                else:
                                    
                                    # Info
                                    GetException(' -------> WARNING: File: ' + sFileName_SELECT + '  --- VARIABLE NOT FOUND --- INIT VARIABLE UNDEFINED!' ,2,1)
                                    
                                    if not sTime_SELECT in a1oData_OUT:
                                        a1oData_OUT[sTime_SELECT] = {}
                                        a1oData_OUT[sTime_SELECT]['GeoX'] = a2dGeoX
                                        a1oData_OUT[sTime_SELECT]['GeoY'] = a2dGeoY
                                        a1oData_OUT[sTime_SELECT]['GeoZ'] = a2dGeoZ
                                        a1oData_OUT[sTime_SELECT]['Attributes'] = oFileData_SELECT['Attributes']
                                        a1oData_OUT[sTime_SELECT]['GeoInfo'] = {}
                                        a1oData_OUT[sTime_SELECT]['GeoInfo']['GeoBox'] = oFileData_SELECT['GeoInfo']['GeoBox']
                                        a1oData_OUT[sTime_SELECT]['GeoInfo']['GeoRef'] = oFileData_SELECT['GeoInfo']['GeoRef']
                                    if not sVarName_SELECT in a1oData_OUT[sTime_SELECT]: a1oData_OUT[sTime_SELECT][sVarName_SELECT] = {}
                                
                                    a2dData_SELECT_UNDEF = np.zeros([a2dGeoZ.shape[0],a2dGeoZ.shape[1]]); a2dData_SELECT_UNDEF[:] = -9999.0
                                    a1oData_OUT[sTime_SELECT][sVarName_SELECT] = a2dData_SELECT_UNDEF
                            ...
                            # Info
                            oLogStream.info(' -------> Variable: ' + sVarName_CHECK + ' Time_IN:' + sTime_IN + ' Time_SELECT: NOT FOUND')
                            oLogStream.info(' -------> Open File: ' + sFileName_SELECT + ' ... SKIPPED')
                            GetException(' -------> WARNING: Open File: ' + sFileName_SELECT + ' ... SKIPPED --- DATA NOT FOUND --- INIT VARIABLE UNDEFINED!' ,2,1)
...
#                             a1oData_OUT[sTime_SELECT][sVarName_SELECT] = a2dData_SELECT_UNDEF
                        ...
                        oLogStream.info(' -------> Variable: ' + sVarName_CHECK + ' Time_IN:' + sTime_IN + ' Time_SELECT: NOT FOUND')
                        oLogStream.info(' -------> Open File: NOT SELECT ... FAILED')
                        GetException(' -------> WARNING: Open File: ' + sFileName_SELECT + ' ... SKIPPED --- FILE NOT FOUND --- INIT VARIABLE UNDEFINED!' ,2,1)
...
#                         a1oData_OUT[sTime_SELECT][sVarName_SELECT] = a2dData_SELECT_UNDEF
                ...
                # Info
                if sVarType_IN == 'OBS':
                    a1sVarName_SELECT = list(set(a1sVarNameOBS_SELECT))
                elif sVarType_IN == 'FOR':
                    a1sVarName_SELECT = list(set(a1sVarNameFOR_SELECT))
                ...
                a1sTime_SELECT_OBS = []; a1sTime_SELECT_FOR = []; a1oTime_SELECT_TOT = []
                for iTime_OUT, sTime_OUT in enumerate(sorted(a1oData_OUT)):
                    ...
                    if sVarType_IN == 'OBS':
                        ...
                        a1sTime_SELECT_OBS.append(sTime_OUT)
                    else:pass
                    
                    if sVarType_IN == 'FOR' and iTime_OUT == 0:
                        iTimeIndex_OUT = a1oTimeStep_IN.index(sTime_OUT)
                        a1sTime_SELECT_FOR.append(sTime_OUT)
                    else:
                        a1sTime_SELECT_FOR.append(sTime_OUT)
                    #-------------------------------------------------------------------------------------
                    ...
                    try:
                        ...
                        # Check if all variable(s) in SELECT are available for each time step
                        iVar_CHECK = False
                        a1sDataKeys_OUT = oData_OUT.keys()
                        iVar_CHECK = set(a1sVarName_SELECT).issubset(set(a1sDataKeys_OUT)) 
                        a1sVarName_UNCHECK = list(set(a1sVarName_SELECT) - set(a1sDataKeys_OUT))
                        
                        # Info about variable(s9 availability
                        oLogStream.info( ' -------> Data Step: ' + sTime_OUT )
                        oLogStream.info( ' -------> Variable defined in data: '+ str(a1sVarName_SELECT) ) 
                        oLogStream.info( ' -------> Variable missed in data: '+ str(a1sVarName_UNCHECK) ) 
                        if sVarType_IN == 'OBS':
                            if a1sVarName_UNCHECK:
                                GetException(' -------> WARNING: Variable missed in data ---> Set to NULL',2,1)
                                a1sVarName_UNCHECK = []
                            else:pass
                        elif sVarType_IN == 'FOR':
                            if a1sVarName_UNCHECK:
                                GetException(' -------> WARNING: Variable missed in data ('+str(a1sVarName_UNCHECK) +')',2,1)
                            else:pass
                            
                        # All variable are CHECK
                        if not a1sVarName_UNCHECK:
                        
                            # Save data in output file
                            oFileDrv_OUT = Drv_Model_HMC_IO(sFileName_OUT, self.oDataInfo, self.oDataStatic)
                        ...
                        # One or more variable(s) are UNCHECK
                        elif a1sVarName_UNCHECK:
                            
                            # Info
                            a1oFileName_OUT.append(sFileName_OUT); a1oFileExist_OUT.append('Skip'); 
                            a1oFileTime_OUT.append(sTime_OUT); a1oFileType_OUT.append(sVarType_IN)
                            a1oFileTime_CHECK.append(sTime_CHECK); a1oFileStatus_OUT.append('SKIP')

                            oLogStream.info( ' -------> Save File: ' + sFileName_OUT + ' ... SKIPPED --- SOME VARIABLE(S) ARE UNAVAILABLE ('+str(a1sVarName_UNCHECK)+')!')
                            GetException(' -------> Save File: ' + sFileName_OUT + ' ... SKIPPED --- SOME VARIABLE(S) ARE UNAVAILABLE ('+str(a1sVarName_UNCHECK)+')!' ,2,1)
                            oLogStream.info( ' ====> TIME STEP OUTPUT: ' + sTime_OUT + ' ... SKIPPED')
                            
                            a1sVarStatus_SELECT[iTimeIndex_OUT] = None
                    except:
                ...
                #-------------------------------------------------------------------------------------
                # Merge dictionary
                oTimeStep_IN = set(a1oTimeStep_IN)
                if sVarType_IN == 'OBS':
                    oTime_SELECT_OBS = set(a1sTime_SELECT_OBS)
                    oTime_SELECT_NREP = oTime_SELECT_OBS - oTimeStep_IN
                    if len(oTime_SELECT_NREP) > 0:
                        a1oTime_SELECT_TOT = sorted(a1oTimeStep_IN + list(oTime_SELECT_NREP))
                    else:
                        a1oTime_SELECT_TOT = a1oTimeStep_IN
                    
                elif sVarType_IN == 'FOR':
                    oTime_SELECT_FOR = set(a1sTime_SELECT_FOR)
                    oTime_SELECT_NREP = oTime_SELECT_FOR - oTimeStep_IN
                    if len(oTime_SELECT_NREP) > 0:
                        a1oTime_SELECT_TOT = sorted(a1oTimeStep_IN + list(oTime_SELECT_NREP))
                    else:
                        a1oTime_SELECT_TOT = a1oTimeStep_IN
                ...
                if os.path.exists(sFileHistory_PS):
                    ...
                    for iI in range(0, len(a1iIndexTime_IN)):
                        ...
                        a1oTime_SELECT_TOT.append(sTime_OUT)
        ...
        self.a1oTime_SELECT_TOT = a1oTime_SELECT_TOT
        #-------------------------------------------------------------------------------------
