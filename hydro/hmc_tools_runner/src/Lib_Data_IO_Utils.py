...
#--------------------------------------------------------------------------------
# Method to delete folder with time older than check time (in path name)
def deleteFolderOld(sPath, oTime):
    
    import shutil
    import dateutil.parser as dparser
    
    # Get time 
    oTime = oTime.replace(hour=0, minute = 0, second = 0, microsecond = 0)
    
    # Cycle(s) on folders tree
    for sRoot, sDirs, sFiles in os.walk(sPath):
        
        try:
            # Define time to check
            sString_CHECK = sRoot.replace('/','')
            oTime_CHECK = dparser.parse(sString_CHECK, yearfirst=True, fuzzy=True)
            oTime_CHECK = oTime_CHECK.replace(hour = 0, minute = 0, second = 0, microsecond = 0)
            
            if oTime_CHECK < oTime:
...
#--------------------------------------------------------------------------------
# Method to delete empty folder in a folder tree
def deleteFolderEmpty(sPath):
    
    # Cycle(s) on folders tree
    for sRoot, sDirs, sFiles in os.walk(sPath):
        
        # rmdir does not remove not empty folder
        try:
            os.rmdir(sRoot)
...
#--------------------------------------------------------------------------------
# Method to search folder with a defined string
def searchFolder(sPath, sTag):
    
    a1oSelFolder = []
    for sRoot, sDirs, sFiles in os.walk(sPath):
    
        if sTag in sRoot:
            a1oSelFolder.append(sRoot)
        else:pass
